...
# -*- coding: utf-8 -*-
"""
Created on Thu May  2 16:20:50 2019
...
BALLON_WEIGHTS_PATH = "/path/mask_rcnn_balloon.h5"  # TODO: update this path
...
# https://github.com/matterport/Mask_RCNN/releases
# weights_path = "/path/mask_rcnn_balloon.h5"

weights_path = os.path.join(ROOT_DIR, "weights/mask_rcnn_balloon.h5")
...
#%% Color Splash
# This is for illustration. You can call balloon.py with the splash option to 
# get better images without the black padding.
...
####################################################
'''
The Region Proposal Network (RPN) runs a lightweight binary classifier on a 
lot of boxes (anchors) over the image and returns object/no-object scores. 
Anchors with high objectness score (positive anchors) are passed to the stage 
two to be classified.

Often, even positive anchors don't cover objects fully. So the RPN also 
regresses a refinement (a delta in location and size) to be applied to the 
anchors to shift it and resize it a bit to the correct boundaries of the object.
'''
#%% 1.a RPN Targets
'''
...
#%%
# Display positive anchors before refinement (dotted) 
# and after refinement (solid).
...
# the previous layer and runs the mask head to generate segmentation masks for 
# every instance.
#%% 3.a Mask Targets
